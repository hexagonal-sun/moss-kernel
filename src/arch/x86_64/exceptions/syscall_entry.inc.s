...
.extern handle_syscall_wrapper

# ExceptionState layout on stack:
# 0..128   -> regs[0..15] (16 * 8 = 128 bytes)
# 128      -> rip (8)
# 136      -> rsp (8)
# 144      -> rflags (8)
# Total: 152 bytes. We use 160 for alignment.

syscall_entry:
    # 1. Swap GS to get kernel GS base
    swapgs

    # 2. Save user RSP and load kernel RSP
    # We assume GS points to a struct:
    # GS:[0] -> kernel stack top
    # GS:[8] -> temporary storage for user RSP
