...
pub async fn sys_copy_file_range(
    ...
    // Offset path: at least one of the offsets was provided. Use readat/writeat
    // with a kernel buffer and manual offset maintenance.
    ...
    // We'll use a scratch page as the backing store for a kernel pipe buffer.
    // This keeps the implementation simple and avoids user buffers.
    let mut pg = ClaimedPage::alloc_zeroed()?;
    let kslice = pg.as_slice_mut();
    ...
    while remaining > 0 {
        let chunk_sz = core::cmp::min(kslice.len(), remaining);
