...
pub struct EpollInstance {
    ...
    interest_list: Mutex<BTreeMap<Fd, EpItem>>,
}
...
pub async fn sys_epoll_ctl(epfd: Fd, op: i32, fd: Fd, event: TUA<EpollEvent>) -> Result<usize> {
    ...
    match op {
        EPOLL_CTL_ADD => {
            ...
            let mut map = epi.interest_list.lock().await;
        ...
        EPOLL_CTL_MOD => {
            let ev: EpollEvent = copy_from_user(event).await?;
            let mut map = epi.interest_list.lock().await;
            let entry = map.get_mut(&fd).ok_or(FsError::NotFound)?;
...
pub async fn sys_epoll_pwait(
    ...
    let items: Vec<EpItem> = {
        let map = epi.interest_list.lock().await;
